...
export function buildSourceMessage(targetUserIds, {
  marker,
  reactionName,
  channelMention = true,
  flexUrl = "https://www.flex.team/",
} = {}) {
  const mentions = [...new Set(targetUserIds)].map((userId) => `<@${userId}>`).join(" ");
  const lines = [
    `*${marker}*`,
    channelMention ? "<!channel>" : null,
    mentions,
    "안녕하세요! 각 팀 리드께서는 이번 주 팀원 휴가 및 결재 승인 현황을 Flex에서 확인해 주세요.",
    "",
    "1. 위클리 미팅 전 팀원 휴가 현황 확인",
    "2. 들어온 워크플로우 결재 요청 승인 여부 확인",
    `3. <${flexUrl}|Flex - 팀원 휴가 및 워크플로우 승인하기>`,
    "",
    `확인 완료 후 이 메시지에 :${reactionName}: 리액션을 남겨주세요.`,
  ];
